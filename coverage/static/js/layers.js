...
function setStatesAfterLoad(map, layer) {
  ...
  if (map.isSourceLoaded(layer.source)) {
      ...
      var metaArticleIds = store.filteredMetadata(filter)
      var articleIds = metaArticleIds.filter(value => pointsArticleIds.includes(value));
      setStates(layer, articleIds);
  } else {
      map.on('sourcedata', setAfterLoad);
  }
}

function setStates(layer, articleIds) {
    return
  
    var sourceFeatures = map.querySourceFeatures(layer.source)

    if (sourceFeatures.length == 0) {
      return
    }

    var polyCount = store.polyDataCount(filter)
    polyLayerCount = polyCount[layer.key]

    for (row in sourceFeatures) {
        sourceFeature = sourceFeatures[row]
        if (sourceFeature) {
            var geoId = parseInt(sourceFeature.id)            
            var layerCount = 0
            if (polyLayerCount[geoId]) {
              var articles = articleIds.filter(x => polyLayerCount[geoId].includes(x))
              layerCount = articles.length   
